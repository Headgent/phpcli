...
> 💡 **Configuration Philosophy:** Build ARGs configure image contents (versions, users, packages). Runtime settings have fixed defaults in the image and are overridden via `docker run -e VAR=value`. No rebuild needed for runtime changes.
